...
/* Prefix-match context for efile_read_info directory detection. */
...
static void list_dir_collect(gleepack_index_entry_t *entry, void *arg) {
    ...
    ctx->names[ctx->count++] = gleepack_strndup(child, name_len);
}
...
posix_errno_t efile_read_info(const efile_path_t *path, int follow_links, efile_fileinfo_t *result) {
    ...
    if (is_gleepack_path(path)) {
        ...
        if (entry != NULL) {
            ...
            result->access = EFILE_ACCESS_READ;
            result->mode   = _S_IFREG | _S_IREAD;
            /* Propagate user bits to group/other */
            result->mode  |= (result->mode & 0700) >> 3;
            result->mode  |= (result->mode & 0700) >> 6;
            result->links  = 1;
            return 0;
        ...
        if (ctx.found) {
            ...
            result->type   = EFILE_FILETYPE_DIRECTORY;
            result->access = EFILE_ACCESS_READ;
            result->mode   = _S_IFDIR | _S_IREAD | _S_IEXEC;
            /* Propagate user bits to group/other */
            result->mode  |= (result->mode & 0700) >> 3;
            result->mode  |= (result->mode & 0700) >> 6;
            result->links  = 1;
