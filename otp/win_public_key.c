 ...
 *
 * gleepack patch: replaced #ifdef WINVER with #ifdef _WIN32 throughout.
 * WINVER is not defined in our static NIF build flags (-D_WIN32 etc.) because
 * it is normally set by <windows.h>, but public_key.c checks the guard BEFORE
 * including windows.h (chicken-and-egg).  _WIN32 is always defined by MSVC for
 * Windows targets and is safe to use as the guard here.
 *
 * Also added `static` to the os_cacerts definition to match the forward
 * declaration on line 42; MSVC (unlike GCC/Clang) rejects a static declaration
 * paired with a non-static definition (C2129).
 */
